...
from uma_mot.tracker.Siamese_inference import inference_wrapper
from uma_mot.tracker.Siamese_inference.Siamese_tracker import Siamese_Tracker
from uma_mot.tracker.Siamese_utils.misc_utils import load_cfgs, get_center, auto_select_gpu
...
class MOT_Tracker:
    ...
    def associate_detections_to_trackers(self, detections, trackers, score_threshold, cos_matrix=None):
        ...
        row_idxs, col_idxs = linear_assignment(-association_matrix)
        ...
        for row, col in zip(row_idxs, col_idxs):
            if (association_matrix[row, col] < score_threshold):
